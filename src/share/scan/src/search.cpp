...
private:
   ...
   static Depth extend_hcp (Move mv, const Local & local);
...
public:
   ...
   const Search_Output & so() const { return *m_so; }
};
...
void search(Search_Output & so, const Node & node, const Search_Input & si) {
   ...
   if (var::Book && si.book) {
      ...
      if (ply < var::Book_Ply && book::probe(node, Score(margin), mv, sc)) {
         so.new_best_move(mv, sc);
         return;
      }
   }
   ...
   if (si.nodes == 1) {
      ...
      sg.sl(ID_Main).search_all_try(node, list, Depth(1));

      const double k = 1.5;
      int index = list::pick(list, k);

      so.new_best_move(list[index], Score(list.score(index)));
...
void Search_Local::search_all_try(const Node & node, List & list, Depth depth) {
   ...
   try {
      search_all(node, list, depth, Ply_Root, true);
   }
   catch (const Abort &) {
      pop_sp(m_sg->root_sp());
      assert(m_stack.empty());
...
void Search_Local::search_asp(const Node & node, const List & list, Depth depth, Ply ply, bool prune) {
   ...
   if (m_sg->si().move && depth >= 4 && score::is_eval(last_score)) {
...
Score Search_Local::search(const Node & node, Score alpha, Score beta, Depth depth, Ply ply, bool prune, Move skip_move, Line & pv) {
...

   {
      ...
      if (G_TT.probe(key, tt_move, tt_score, tt_flag, tt_depth)) {
         ...
         if (!(!m_sg->si().move && local.pv_node) && tt_depth >= local.depth && tt_score != score::None) {
   ...
   if (local.ply >= m_sg->si().ply) return leaf(eval(node), local.ply);
...
Score Search_Local::search_move(Move mv, const Local & local, Line & pv) {
   ...
   // handicap >= 2: no search extensions
   bool extensions = m_sg->si().handicap < 2;

   Depth ext = extensions ? (m_sg->si().handicap == 0 ? extend(mv, local) : extend_hcp(mv, local)) : Depth(0);
   Depth red = extensions ? reduce(mv, local) : Depth(0);
   ...
   if (local.pv_node
    ...
    && ext == 0
      ...
      if (extensions && sc <= new_alpha) ext = Depth(1);
   }
...
Score Search_Local::qs(const Node & node, Score alpha, Score beta, Depth depth, Ply ply, Line & pv) {
   ...
   int handicap = m_sg->si().handicap;

   // handicap >= 4: no qs, no extensions
   if (handicap > 4 || ply >= m_sg->si().ply) return leaf(eval(node), ply);
...
Depth Search_Local::extend_hcp(Move mv, const Local & local) {
...
void Search_Local::inc_node() {
   ...
   m_node += 1;
   if (m_node >= m_sg->si().nodes && m_sg->so().move != move::None) m_sg->abort();
